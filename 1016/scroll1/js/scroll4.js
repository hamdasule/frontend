$(function(){
    ...
    $(window).scroll(function(){
        ...
        $('#float_div').stop().animate({top:f_top+sct},400);
    ...
    //크롬에서 jquery.easing.1.3.js 효과검색해서 사용가능
    ...
    $('#popup').draggable(); //드래그 가능 https://jqueryui.com/

    //나중에 pop = no를 넣어 하루동안 저장 예정
    //처음에는 pop변수도 없고 no 도 없음
    if($.cookie('pop')!='no'){
        $('#popup').show(); //쿠키 pop변수에 no란 단어가 없다면 팝업을 보여준다.

    }
    $('#popup area:eq(0)').click(function(){
        $('#popup').fadeOut('fast');
        //닫기번튼 클릭하면 팝업을 서서히 숨긴다.
    });
     $('#popup area:eq(1)').click(function(){
        $.cookie('pop','no',{expires:1});
        $('#popup').fadeOut('fast');
    });
//f12어플리케이션 쿠키에서 삭제하면 다시 창이뜸
...
//<다른창>
    ...
    //팝업변수에 none이 저장되어있으면 $('#notice_wrap')을 숨겨라
    ...
    function closePop(){
        if(chk.is(':checked')){//chk에 체크가 되어있으면
